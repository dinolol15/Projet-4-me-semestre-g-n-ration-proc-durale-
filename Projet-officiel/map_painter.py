...
class DrawingModule:
    """
    Better off with a class than this war crime of globals
    """
    ...
    def __post_init__(self):
        ...
        pen_tool_icon = Image(self.cam, "UI", layer=2, position=(5, 755), centered=False, size=0.1,
        ...
        tool_pointer_wrap = self.cam.DynamicWrapper(self.tool_pointer, self.cam, tool_pointer_update)
    ...
    def update(self, dt):
        ...
        def toolbar_action(n: int):
            ...
            if n == 0: #draw tool
                ...
                self.square.update_data_values(map_dimensions=self.map_size, map_pixel_size=self.tile_size)
                self.memo.max_id = self.map_size
                self.memo.reinit()
        ...
        if self.cam.mouse_left:
            if self.mouse_pos()[0] < 50 and self.toolbar_range[0] < self.mouse_pos()[1] < self.toolbar_range[1]:
...
def main():
    ...
    s = Square(
        ...
        "game",
        layer=0,
        map_dimensions=map_size,
        map_pixel_size=tile_size,
        size=map_base_size
    )
    s.rand_test()
    ...
    tools_back = pyglet.shapes.Rectangle(
        ...
        toolbar_range[0],
        50,
        toolbar_range[1]-toolbar_range[0],
        color=(255, 255, 255),
        batch=cam.batch_UI
    )
    cam.window_UI_static.append(tools_back)
    #this single line above is for the only purpose of avoiding a pylint error
    ...
    #the loop of pyglet + pylint doesn't understand that pyglet passes dt on this function
    #so please spare the conv points
    def update(dt):
        global map_size
        global s
